        ...
        )))
        {
            if (GenUtils.chance(random, 1, 10)) { // Grass
                switch (random.nextInt(3)) {
                    ...
                    case 2 -> V_1_21_5.wildflowers(random, data, rawX, surfaceY + 1, rawZ);
                }
            }
        }
    }
                                   ...
                                   @NotNull PopulatorDataAbstract data)
    {
        ...
        for (SimpleLocation center : flowerCenters) {
            ...
            BlockUtils.lambdaCircularPatch(
                    ...
                    new SimpleBlock(data, center.getX(), 0, center.getZ()).getGround(),
                    (b) -> {
                        if (data.getBiome(b.getX(), b.getZ()) == getBiome() && hashRand.nextInt(4) == 0) {
