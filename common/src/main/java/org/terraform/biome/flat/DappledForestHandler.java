        ...
        if (data.getType(rawX, surfaceY, rawZ) == Material.GRASS_BLOCK) {
            if (GenUtils.chance(random, 1, 10)) {
                ...
                switch(random.nextInt(3)){
                    case 0 -> org.terraform.tree.PlantBuilder.GRASS.build(data, rawX, surfaceY + 1, rawZ);
                    case 1 -> BlockUtils.pickFlower().build(data, rawX, surfaceY + 1, rawZ);
                    case 2 -> org.terraform.tree.PlantBuilder.BUSH.build(data, rawX, surfaceY + 1, rawZ);
                }
            }
        }
    }
