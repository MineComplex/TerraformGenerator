...
public class OrePopulator {
    ...
    public static final ConcurrentHashMap<TerraformWorld, FastNoise> ORE_NOISE_CACHE = new ConcurrentHashMap<>();
    ...
    public void placeOre(int seed, @NotNull PopulatorDataAbstract data, int coreX, int coreY, int coreZ) {
        ...
        FastNoise noise = ORE_NOISE_CACHE.get(data.getTerraformWorld());
        if (noise == null) {
            noise = new FastNoise(seed);
            noise.SetNoiseType(NoiseType.Simplex);
            noise.SetFrequency(0.09f);
            ORE_NOISE_CACHE.put(data.getTerraformWorld(), noise);
        }
